...
class MultiPosPadUNet3D(nn.Module):
    def __init__(self, n_classes, emb_shape, in_ch, size=32):
        self.n_classes = n_classes
        self.in_ch = in_ch
        super(MultiPosPadUNet3D, self).__init__()

        self.emb_shape = torch.as_tensor(emb_shape) # TODO
        self.pos_encoder_0 = PositionalEmbedding(size * 16, max_len=15**3)

        self.pos_encoder_1e = nn.ConvTranspose3d(size * 16, size * 8, kernel_size=2, stride=2)
        self.pos_encoder_2e = nn.ConvTranspose3d(size * 8, size * 4, kernel_size=2, stride=2)
        self.pos_encoder_3e = nn.ConvTranspose3d(size * 4, size * 2, kernel_size=2, stride=2)

        self.pos_encoder_1d = nn.ConvTranspose3d(size * 16, size * 8, kernel_size=2, stride=2)
        self.pos_encoder_2d = nn.ConvTranspose3d(size * 8, size * 4, kernel_size=2, stride=2)
        self.pos_encoder_3d = nn.ConvTranspose3d(size * 4, size * 2, kernel_size=2, stride=2)
    ...
    def forward(self, x, emb_codes):
        ...
        emb_pos_3e = self.pos_encoder_3e(emb_pos_2e)

        emb_pos_1d = self.pos_encoder_1d(emb_pos_0)
        emb_pos_2d = self.pos_encoder_2d(emb_pos_1d)
