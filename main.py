...
def main():
    ...
    writer = SummaryWriter(log_dir=os.path.join(config['tb_dir'], args.experiment_name), purge_step=current_epoch)
    ...
    if train_config['do_train']:
        ...
        warm_up[0:int(train_config['epochs'] * train_config.get('warm_up_length', 0.35))] = np.linspace(0, 1, num=int(train_config['epochs'] * train_config.get('warm_up_length', 0.35)))
        ...
        for epoch in range(current_epoch, train_config['epochs']):
            ...
            if epoch % train_config.get('validate_after_iters', 2) == 0:
                val_metric = test(model, test_loader, loss, epoch, writer, evaluator, warm_up[epoch])
    ...
    # final test
    test_score = test(model, test_loader, loss, train_config['epochs'] + 1, writer, evaluator, 1, dumper=vol_writer)
...
if __name__ == '__main__':
    ...
    BASE_DIR = r'Y:\work\results' if socket.gethostname() == 'DESKTOP-I67J6KK' else r'/nas/softechict-nas-2/mcipriano/results/maxillo/3D'

    arg_parser = argparse.ArgumentParser()
    arg_parser.add_argument('--experiment_name', default="test", help="name of this experiment folder")
    ...
    project_dir = path.join(BASE_DIR, args.experiment_name)
    log_dir = pathlib.Path(os.path.join(project_dir, 'logs'))

    if args.experiment_name != 'test' and path.exists(project_dir):
    ...
    if not args.verbose:
        ...
        utils.set_logger()  # logging to console <- verbose
