...
class TransformerBTS(nn.Module):
    ...
    def forward(self, x, pos, auxillary_output_layers=[1, 2, 3, 4]):
...
def TransBTS(num_classes, img_dim, _conv_repr=True, _pe_type="learned"):
...
if __name__ == '__main__':
    with torch.no_grad():
        x = torch.rand((1, 3, 120, 120, 120)).cuda()
        _, model = TransBTS(num_classes=2, img_dim=120, _conv_repr=True, _pe_type="learned")
        model.cuda()
        y = model(x)
        print(y.shape)
