...
if __name__ == '__main__':
    ...
    for i, (folder) in tqdm(enumerate(patient_folders), total=len(patient_folders), desc='creating fancy animations'):

        data = np.squeeze(np.load(os.path.join(source_path, str(i), 'input.npy')))
        gt = np.squeeze(np.load(os.path.join(source_path, str(i), 'gt.npy')))
        pred = np.squeeze(np.load(os.path.join(source_path, str(i), 'pred.npy')))

        jaw = Jaw(os.path.join(dataset_folders, str(folder), 'DICOM', 'DICOMDIR'))

        assert np.all(np.squeeze(gt) == np.load(os.path.join(dataset_folders, str(folder), 'gt_2labels.npy'))), 'gt from experiment does not match with the dataset one'
